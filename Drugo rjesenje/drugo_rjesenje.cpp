...
class Diagnostics{ //Pomoćna klasa, slobodno je proširiti
    private:
        ...
        u32 contiguous;
    public:
        int GetWaitingProcessesNum() const{
            return waiting;
        }
...
struct FreeSpaceReport{
public:
    u32 PagesNum;
    u32 AppropriateSize;
    char SizeChar;
    u32 BiggestContiguous;

    FreeSpaceReport(
        u32 pagesNum, u32 appropriateSize,
        char sizeChar, u32 biggestContiguous
    ){
        PagesNum = pagesNum;
        AppropriateSize = appropriateSize;
        SizeChar = sizeChar;
        BiggestContiguous = biggestContiguous;
    }

    FreeSpaceReport(const FreeSpaceReport& ref){
        PagesNum = ref.PagesNum;
        AppropriateSize = ref.AppropriateSize;
        SizeChar = ref.SizeChar;
        BiggestContiguous = ref.BiggestContiguous;
    }
};
...
class SystemMemory{
    public:
        ...
        void write(int processID, int segmentID, u32 logicalAddress, char value){
            unique_lock<mutex> l(
                mSegments[
                        segmentTables[processID].getEntry(segmentID).m
                    ].m
                );
        ...
        FreeSpaceReport DetermineFreeSpace(){
            FreeSpaceReport fsr(0, 0, 0, 0);

            u32 freeMem = 0; // byte number
            const u32 pageSize = 4096;


            u32 maxContSpace = 0;
            u32 currentContSpace = 0;
            bool cont = true;
            ...
            for(list<Fragment>::iterator it = freeMemory.begin(); it != freeMemory.end(); it++){
                ...
                if(currentContSpace > maxContSpace){
                    maxContSpace = currentContSpace;
                }
                ...
                if(it != --freeMemory.end()){
                    ...
                    cont = next->loc == it->len + it->loc;
                }
            }


            fsr.PagesNum = freeMem / pageSize;
            fsr.BiggestContiguous = maxContSpace / pageSize;

            if (freeMem >= 1024) // KB
            {
                fsr.SizeChar = 'K';

                fsr.AppropriateSize = freeMem / 1024;
            }
            else if (freeMem >= 1024 * 1024)
            {
                fsr.SizeChar = 'M';

                fsr.AppropriateSize = freeMem / (1024 * 1024);
            }
            else {
                fsr.SizeChar = 'B';
                fsr.AppropriateSize = freeMem;
            }
        ...
        void ReportFreeSpace(){
            FreeSpaceReport fsr = DetermineFreeSpace();
            cadh.reportFreeSpace(
                fsr.PagesNum,
                fsr.AppropriateSize,
                fsr.SizeChar,
                fsr.BiggestContiguous
            );
        }
...
class Process{
    public:
        ...
        void run(){
            ...
            SegmentData sd;
            sd.Size = amount;
            sd.TimeStamp = chrono::high_resolution_clock::now().time_since_epoch().count();
            localTable.insert(pair<int, SegmentData>(permanentSegment, sd));
            while(1){
                if(terminate) return;
                ...
                if(step < 7){
                    ...
                    sd.Size = amount;
                    sd.TimeStamp = chrono::high_resolution_clock::now().time_since_epoch().count();
                    localTable.insert(pair<int, SegmentData>(seg, sd));
                    ...
                    int seg = systemMemory.getRandomID(id, generator);
                    uniform_int_distribution<u32> sd(0, localTable[seg].Size - 1);
                    u32 logicalAddress = sd(generator);

                    systemMemory.read(id, seg, logicalAddress);
                    localTable[seg].TimeStamp =
        ...
        map<int, SegmentData>& GetLocalTable(){
            return localTable;
        }

        void EraseSegment(int seg){
            localTable.erase(seg);
        }
    ...
    private:
        ...
        map<int, SegmentData> localTable;
...
void GlobalProcessTableLRU(
    const Diagnostics& diagnostics,
    SystemMemory& systemMemory,
    Process processes[PROCESSES]
){
    while (!systemMemory.GetTerminal())
    {
        if (lruActive && (diagnostics.GetWaitingProcessesNum() == PROCESSES))
        {
            std::cout << "LRU starting" << std::endl;
            std::pair<int, SegmentData> lru;
            int procIndx = -1;

            for (int i = 0; i < PROCESSES; i++)
            {
                auto& localTable = processes[i].GetLocalTable();

                bool first = true;

                for (const std::pair<int, SegmentData>& p : localTable)
                {
                    if (first)
                    {
                        lru = p;
                    ...
                    else if (lru.second.TimeStamp > p.second.TimeStamp)
                    {
                        procIndx = i;
                        lru = p;
                    }
            ...
            if(procIndx != -1){
                std::cout << "LRU succesful" << std::endl;

                systemMemory.deallocate(processes[procIndx].GetId(), lru.first);
                processes[procIndx].EraseSegment(lru.first);
            }
        }
...
int main(int argc, char** argv){
    ...
    thread lruT(GlobalProcessTableLRU, ref(d), ref(mem), processList);
    lruT.detach();
