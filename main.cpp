//PR 106/2020 Radoslav Mastilović, Termin vezbi: Pon 16:00. Radjeno: SOV-AB. Komentari: ...
...
class Diagnostics{ //Pomoćna klasa, slobodno je proširiti
    ...
    public:
		...
		///DODATA FJA  geter da dobijemo koliko procesa ceka da bi nesto radilo na memoriji
		int GetWaiting()
		{
			return waiting;
		}
		...
		//dodali smo timestamp parametar jer smo prosirili strukturu tableentry 302 linija, ovo je potrebno da bi ispisali na konzolu samo kad se alocira taj timestamp
        void allocateMessage(int pid, u32 loc, u32 len, u32 seg,std::time_t timestamp){
            unique_lock<mutex> l(m);
            cout << "Process " << pid << " allocating from " << loc << " to " << loc + len << " ts:"<<timestamp<<endl;//prosiren cout<<
            if(visual){
                int x = loc / 4096;
                int ll = len / 4096;
                for(int i = 0;i < ll;i++){
...
struct TableEntry{ //Jedna stavka u tabeli segmenta
    ...
    u32 len = 0;
    int m = -1;
	std::time_t timestamp = std::time(nullptr);//dodali da bi biljezili kad se zadnji put radilo sa tim segmenom
};
...
class SystemMemory{
    public:
        ...
        char read(int processID, int segmentID, u32 logicalAddress){
			...
			///menjamo timestamp segmenta
			segmentTables[processID].getEntry(segmentID).timestamp=std::time(nullptr);
        ...
        int allocate(int processID, u32 amount){
            if(!segmentTables.count(processID)){ //U slučaju da ovo zovemo prvi put za dati proces, ubacujemo novu tabelu
                ...
                //dodajemo polje timestamp u strukturi tableEntry
				std::time_t t=std::time(nullptr);
				int ret = segmentTables[processID].insertSegment((TableEntry) {.base = (u32)loc, .len  = amount, .m = getFreeMutex(),.timestamp=t});
                //allocateMessage prima dodatni patametar za timestamp, to je ona gore za ispis alokacije
				cadh.allocateMessage(processID, (u32)loc, amount, ret,t);
				...
				//dodajemo polje timestamp u strukturi tableEntry, morali smo dodati ovu liniju jer nam je samo struktura prosirenija za time stamp
				std::time_t t=std::time(nullptr);
                int ret = segmentTables[processID].insertSegment((TableEntry) {.base = (u32)loc, .len  = amount, .m = getFreeMutex(),.timestamp=t});
				//allocateMessage prima dodatni patametar za timestamp
                cadh.allocateMessage(processID, (u32)loc, amount, ret,t);
                return ret;
            }
        }
		...
		//Dodata funkcija
		//prolazi iteratorom kroz free memoriju i samo broji od svake strukture(fragmenta) kolika je len i to vraca
		u32 ReportFree()
		{
			u32 sum=0;
		...
		//funkcija koja trazi maksimalni slobodni fragment(klasicno trazenje maksimuma)
		u32 GetMaxFragment()
		{
			u32 maks=0;
			for(list<Fragment>::iterator it = freeMemory.begin();it != freeMemory.end();it++){
		...
		//nit napravljena u mainu, ulazi u while true, zakuca se i ceka da mi upalimo lru
		//kada upaliomo tada ovu nit i notifikujemo,kada iskljucimo lru opet upada u while(!lru/Active) i ceka tu
		//inace kada ne ceka provjerava da li ceka 4 procesa (PROCESSES) i poziva metodu LruProcess() i notifikuje jednu nit(proces) da krene
		// i pokusa nesto da alocira ili sta vec jer smo mi oslobodili jedan segment\
		//dodatno imamo spavanje 30ms i ako je termina onda zavrsavamo detach nit(to je nit koja radi ovu funkciju)
		void lruRunner()
		{
		    ...
		    while(1){
		        ...
		        cout<<"nit notifikovana"<<endl;
		        if(cadh.GetWaiting()==PROCESSES)
		        {
		        cout<<"nit oslobadja segment"<<endl;
		            LruProcess();
		            cvFree.notify_one();
		        }
		...
		//lruproces funkcija prolazi kroz sve tabele od svakog procesa ima po jedna tabela
		//nakon toga kroz svaku tabelu ide i cuva rime stamp, na kraju pronalazi najmanji timestamp(seghment sa
		//kojim se najdavnije nesto radilo write/read... i njega brisemo sa deallocate
		void LruProcess()
		{
		cout<<"lru process"<<endl;
		    std::time_t oldest;
		    int segId=-1;
		    bool first=true;
		    int procId=-1;
			for(int i=0;i<PROCESSES;i++)
			{
			    ...
			    for (std::map<int,TableEntry>::iterator it=segmentTables[i].table.begin(); it!=segmentTables[i].table.end(); ++it)
                         if(first){
...
class Process{
    public:
        ...
        void run(){
            ...
            while(1){
				////////////////CHECK FREE MEMORY  od ove linije do 943  dodato
				//poziva se nova metoda reportfree koja izbroji koliko je praznih bajtova
				u32 sum=systemMemory.ReportFree(); ///1kb=1024b    1mb=1024 * 1024b
				u32 stranice=0;
				u32 oznaka;

				stranice=sum/4096; //jedna stranica je 4096 pa ovako dobijamo br stranica
				if(sum<1024)//ako je manje od 1024 bajta onda je mijerna jedinica bajt, a ne kB odnosno kako je kod nas k
				{
					oznaka='b';
				}
				else if(sum>=1024 && sum<1024*1024) //ako je suma >=1024 i manje od jednog mb onda je mjerna jedinica kB odnosno kod nas k
				{
					oznaka='k';
					sum=sum/1024;
				}
				else//i ako nije ispunjeno nista od ovoga onda je mb odnosno m jer je char za ispis
				{
					sum=sum/(1024*1024);
					oznaka='m';
				}
				//funkcija koja je potrebna da se to ispise na vizualizatoru(njihova f-ja)
				//poslednji parametar je nasa funkcija koja vraca duzinu najveceg trenutnog fragmenta
				cadh.reportFreeSpace(stranice,sum,oznaka,systemMemory.GetMaxFragment()/4096);
								////////////////CHECK FREE MEMORY
                if(terminate) return;
...
// klikom na f5 poziva ee ova funkcija i postavlja glob prom lruActive na true prije poziva ove funckije
//i kada udje u if notofokuje gore u funkciji lruRunner nit da krene sa radom
void onLRUChanged(){
	cout<<"lru funckija"<<endl;
	if(lruActive)
	{
	cout<<"lru akrivan"<<endl;
		unique_lock<mutex> l(mLru);
		cvLru.notify_one();
...
//funkcija koja se prva iz maina pozove i ona je posrednik do klase SystemMemory i metode lruRunner
